...
from link import Link, Position, Robot
#from se3 import SE3, set_simplify
...
def main():
    ...
    Tl3l2 = inv(Tl1l2.copy())

    Tl1n = Tl2n @ Tl1l2
    Tl3n = Tl2n @ Tl3l2

    eely = Robot(links = [l1, l2, l3], transforms = [Tl1n, Tl2n, Tl3n])


    v = np.array([0, 0, 0])
    subs = {xn: 0, yn: 0, zn: 0, phi: 0, theta: 0, psi: 0, a1: sp.pi/3, a2: 0, a3: sp.pi/3, a4: 0}
    ...
    p1_arrows = ax.quiver(np.zeros(3), np.zeros(3), np.zeros(3), np.zeros(3), np.zeros(3), np.zeros(3), color='r')
    ...
    ax_phi = plt.axes([0.1, 0.01, 0.65, 0.03], facecolor='lightgoldenrodyellow')
    ax_theta = plt.axes([0.1, 0.06, 0.65, 0.03], facecolor='lightgoldenrodyellow')
    ax_psi = plt.axes([0.1, 0.11, 0.65, 0.03], facecolor='lightgoldenrodyellow')
    ...
    slider_theta = Slider(ax_theta, 'Pitch (theta)', -np.pi/2, np.pi/2, valinit=0)
